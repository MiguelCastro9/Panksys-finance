...
public interface FeedbackRepository extends JpaRepository<FeedbackModel, Long> {
    ...
    public List<FeedbackModel> filterFeedbackByStars(@Param("total_stars") Integer totalStars);
    
    @Query(value = "SELECT * FROM feedbacks WHERE user_id = :user_id AND enabled = true", nativeQuery = true)
    public List<FeedbackModel> findAllMyFeedbacks(@Param("user_id") Long userId);
    
    @Query(value = "SELECT * FROM feedbacks WHERE id = :id AND enabled = true", nativeQuery = true)
    public Optional<FeedbackModel> findFeedback(@Param("id") Long id);
}
